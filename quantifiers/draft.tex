...
\end{inparaenum}
From the present point of view, it is not important when exactly in
the past a certain input has been read or when a choice has been made,
but rather the (symbolic) value and its associated data
constraints. Intuitively, existential quantification of these
variables does the job of ignoring the particular moments in the past
when they have been encountered. Formally, to see that the new set of
formulae $U'(\alpha_0), \ldots, U'(\alpha_n)$ complies with Definition
\ref{def:unfolding}, it is enough to notice the following
facts: \begin{compactitem}
...
\item by Definition \ref{def:generalized-lyndon-interpolant}
  ...
  \leftarrow \stamp{\psi}{k+1} \models I_{k+1}\] By slightly abusing
  notation, let $\stamp{I}{n}_\ell$ be the formula in which every
  predicate symbol $\stamp{q}{\ell}$ has been replaced with
  $\stamp{q}{n}$. Then the following entailment holds: 
  ...
  \leftarrow \stamp{\psi}{1} \models \stamp{I}{1}_{k+1}\]
  ...
  \forall y_1 \ldots \forall y_{\#(q)} ~.~ \stamp{q}{0}(\vec{y})
  \leftarrow \stamp{\psi}{1} \models \stamp{J}{1}_{k+1}\] Since $U$
...
such as \cite{ReynoldsKK17}, perform rather well in answering a large
number of queries in this class. Observe, moreover, that coverage does
not need to rely on a complete decision procedure. If the prover fails
in answering the above satisfiability query, then the semi-algorithm
assumes the node is not covered and continues exploring its
successors. Failure to compute complete coverage may lead to
divergence (non-termination) and failure to prove emptiness, but the
soundness of the procedure is preserved. 
