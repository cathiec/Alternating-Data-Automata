...
\proof{ ``$\subseteq$'' Let $\I$ be a minimal interpretation such that
  ...
  the subformula of $\pathform{\A}{w_\Sigma}$ corresponding to the
  ...
  we have $\I \in \sem{\stamp{\psi}{n}}_{w_\Data}$ and let $\K$ be one
  of the minimal interpretations such that $\K \subseteq \I$ and $\K
  \in \sem{\stamp{\psi}{n}}_{w_\Data}$. It is not hard to see that
  $\K$ exists and is unique, otherwise we could take the pointwise
  intersection of two or more such interpretations. We define the
  interpretation $\bar{\K}(q) = \bar{\K}(\stamp{q}{n})$ for all $q \in
  Q$. We have that $\bar{\K} \in \minsem{\psi}_{\nu}$ --- if
  $\bar{\K}$ was not minimal, $\K$ was minimal to start with,
  contradiction. Then we extend the execution $\G$ by appending to
  each node labeled with a configuration $q(d_1, \ldots, d_{\#(q)})$
  the cube $\cube{\bar{\K}}$. Clearly the result is an execution of
  $\A$ over $w$. 

  ``$\supseteq$'' \comment[ri]{todo} \qed}
