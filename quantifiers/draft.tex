...
\emph{monotonic} iff for each $n < m$ we have $\xi(n) \leq \xi(m)$
     ...
     finite-range, we denote by $\xi^{-1}(n) \in \nat$ the maximal
     value $m$ such that $\xi(m)=n$. It is easy to see that $\xi^{-1}$
     is strictly monotonic if $\xi$ is monotonic and finite-range. The
     lemma below gives the proof of correctness for the previous
     construction of non-local GLI.
...
\proof{(\ref{it1:subst-quant}) If $\accform{\alpha}$ is unsatisfiable,
  ...
  \item $\voc{\widehat{\Phi}_0[\tau_{i_1}/x_{i_1}, \ldots,
    ...
    \stamp{X}{0} \cup \set{x_j \mid j < \xi^{-1}(0), Q_j = \exists}$,
    %
