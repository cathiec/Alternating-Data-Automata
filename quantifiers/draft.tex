...
\proof{ 
  ...
  Since $w \in \lang{A}$, by Lemma \ref{lemma:acceptance}, that there
  exists an interpretation $\I$ such that $\I,\data{w} \models
  \accform{\alpha}$. If $\alpha \in \dom(U)$ then one can show, by
  induction on the length $n\geq0$ of $w$, that $\pathform{\alpha}
  \models \stamp{U(\alpha)}{n}$, thus $\I,\data{w} \models
  \stamp{U(\alpha)}{n}$. But $\I,\data{w} \models \bigwedge_{q \in Q
  ...
  $\stamp{q}{n}$ with $q$ in the previous formula, we obtain that
