...
replacing in $\accform{\alpha}$: \begin{compactitem}
\item each occurrence of a predicate
  $\stamp{q}{i}(t_1,\ldots,t_{\#(q)})$ by the formula
  $\stamp{\psi}{i+1}[t_1/y_1,\ldots,t_{\#(q)}/y_{\#(q)}]$, where
  $q(y_1,\ldots,y_{\#(q)}) \arrow{a_{i+1}(X)}{} \psi \in \Delta$ is the
  unique transition rule for $q$ and $a_i$, for all $i=0,\ldots,n-1$,
%
\item each occurrence of a predicate $\stamp{q}{n}$, such that $q \in
  Q \setminus F$, by $\top$, and
%
\item each occurrence of a predicate $\stamp{q}{n}$, such that $q \in
  F$, by $\bot$.
\end{compactitem}
