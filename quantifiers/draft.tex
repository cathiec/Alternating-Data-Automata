...
even number of negations. A formula is in \emph{prenex form} if it is
of the form $\varphi = Q_1x_1 \ldots Q_nx_n ~.~ \phi$, where $\phi$
has no quantifiers. In this case we call $\phi$ the \emph{matrix} of
$\varphi$. Every first-order formula can be written in prenex form, by
renaming each quantified variable to a unique name and moving the
quantifiers upfront.
...
spuriousness of the given (finite) path. The semi-algorithm uses
moreover a coverage relation to ensure that a spurious counterexample
is never explore twice, thus making progress towards either finding a
real counterexample or a proof of emptiness. In the rest of this
section, we discuss the generation of interpolants
(\S\ref{sec:interpolants}) and the coverage (\S\ref{sec:coverage})
used by our emptiness checking semi-algorithm (\S\ref{sec:impact}). 

\subsection{Interpolants}\label{sec:interpolants}
...
certain step of a given execution. As we discuss next, it is useful to
distinguish between
\begin{inparaenum}[(i)]
...
\begin{lemma}\label{lemma:subst-quant}
  ...
  \item\label{it1:subst-quant} there exist witness terms $\tau_{i_1},
    ...
    %
  \item\label{it2:subst-quant} there exists a GLI $(I_0, \ldots, I_n)$
    for $\alpha$, such that $\voc{I_k} \cap \stamp{Q}{\leq n}
    \subseteq \stamp{Q}{k}$ and $\voc{I_k} \cap \stamp{X}{\leq n}
    \subseteq \stamp{X}{\leq k}$, for all $k \in [n]$.
  \end{compactenum}
\end{lemma}
