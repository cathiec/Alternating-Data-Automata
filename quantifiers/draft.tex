...
\begin{proposition}\label{prop:substitution}
  ...
  in which the boolean variable $q$ occurs positively, for any formula
  $\psi$, the following equivalence holds:
  \[Q_1 x_1 \ldots Q_n x_n ~.~ \phi[\psi/q] \iff 
  Q_1 x_1 \ldots Q_n x_n \exists q ~.~ \phi \wedge (q \rightarrow \psi)\]
\end{proposition}
\proof{ Let $\nu$ be an arbitrary valuation of the variables in
  $\fv{}{\phi} \cup \fv{}{\psi}$. It is sufficient to prove that $\nu
  \models \phi[\psi/q]$ if and only if $\nu \models \exists q ~.~ \phi
  \wedge (q \rightarrow \psi)$.
