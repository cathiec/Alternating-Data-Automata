...
spuriousness of the given (finite) path. The emptiness checking
semi-algorithm uses moreover a coverage relation to ensure that a
spurious counterexample is never explore twice, thus making progress
towards either finding a real counterexample or a proof of emptiness.

Our implementation is a version of the \impact~ lazy annotation
semi-algorithm \cite{McMillan06}. The detailed pseudocode has been
given for a simpler model of boolean alternating automata extended
with variables, in \cite{IosifXu18} and is reported, for
self-completeness, in Appendix \ref{app:pseudocode}. For the time
being, we shall content ourselves of a high-level definition of the
data structure used to prove emptiness of an automaton.
...
\begin{definition}\label{def:unfolding}
  ...
  \Sigma^* \rightharpoonup_{\mathit{fin}} \posforms(Q,X)$, such
  that: \begin{compactenum}
  ...
  \item\label{it3:unfolding} for each sequence $\alpha a \in \dom(U)$,
    ...
    \forall y_{\#q} ~.~ \stamp{q}{0}(\vec{y}) \arrow{a(X)}{}
...
essentially build unfoldings of automata trying to discover
counterexamples for emptiness. If the automaton $\A$ in question is
non-empty, a systematic enumeration of the input event
sequences\footnote{For instance, using breadth-first search.} from
$\Sigma^*$ will suffice to discover a word $w \in \lang{\A}$, provided
that the first-order theory of the data domain $\Data$ is decidable
(Lemma \ref{lemma:acceptance}). However, if $\lang{\A} = \emptyset$,
the enumeration of input event sequences may, in principle, run
...
A lazy annotation semi-algorithm can stop and report emptiness if it
succeeds in building a closed and safe unfolding of the automaton. For
instance, the \impact~ semi-algorithm \cite{McMillan06} 


The higher complexity of the first-order alternating automata model
versus the automata considered in \cite{IosifXu18}, which uses boolean
constants for control states and whose transition rules have no
quantifiers, manifests itself within the method used to generate the
labels of the unfolding, which is discussed in the rest of this
section.

\subsection{Interpolant Generation}\label{sec:interpolants}
