...
\S\ref{sec:emptiness} can extend our previous \emph{generic register
  automata} \cite{HolikIosifRogalewiczVojnar18} inclusion checking
framework, by allowing monitor (right-hand side) automata to have
local variables, that are not visible in the language\footnote{We
  conjecture that hidden computations using invisible variables
  (clocks) are the reason for which timed and register automata cannot
  be complemented within their class.}.
...
\proof{
\qed}
As before, this result enables using the semi-algorithm from
...
automata with predicates of arity greater than one, other than the
explicit enumeration of cubes. Moreover, no method for dealing with
emptiness in the presence of transition quantifiers is known to exist.
