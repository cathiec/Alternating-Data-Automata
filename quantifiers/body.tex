...
instead of formulae. In contrast, a finite description of a first
order alternating automaton must be symbolic (in terms of formulae)
because here, a given configuration can have infinitely many
successors.
...
input word. Consider, for instance, an automaton with the following
transition rules and final state $q_f$:
\[\begin{array}{rclcrclcrcl}
...
\wedge q_2(x_2) \wedge x_1+x_2 = 0$. Note that a control-local formula
...
\begin{example}[Contd. from Examples \ref{ex:quant-elim} and
  ...
  $z_2$. Replacing $z_2$ with $\tau_2$ in the matrix of
...
when exactly in the past these values have been seen.
