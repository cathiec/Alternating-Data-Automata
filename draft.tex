...
\proof{ Let $\pi(u) \equiv \theta_0(Q_0) \wedge \theta_1(Q_0 \cup
  ...
  (\ref{it2:progress}) If $\tuple{\top,I_1,\dots,I_{n+1},\bot}$ is an
   ...
   $\I,\beta_n\cup\nu_{n-1}\cup\nu_n \models
   \abs{\Delta^n(\AbsPost{\A}(\iota,a_1\ldots
     a_{n-1}),a_n)}[Q_{n}/Q]$, for some valuations $\beta_{n} : Q_{n}
   \rightarrow \booli$ and $\nu_i : \vec{x}_i \rightarrow \Data^\I$,
   for $i=n-1,n$, then we build a valuation $\beta_{n-1} : Q_{n-1}
   \rightarrow \booli$ such that $\I,\beta_{n-1}\cup\beta_n \cup
...
labels. Then $\Lambda(n)$ becomes equivalent to $\bot$.
...
Finally, we expand $n$ (lines
\ref{ln:expand-begin}-\ref{ln:expand-end}) by creating a new node for
each successor $s$ via the event $a \in \Sigma$ and inserting it
into the worklist. The label of the new node is $\top$, its
replacement set $R(s)$ is computed based on $R(n)$ and the label of
the edge $(n,a,s)$ is $\Theta(n,a,k)$, where $k$ is the depth of $n$
in $\Art$.

The following lemma states that Algorithm \ref{alg:impact} builds an
over-approximation of the concrete runs of $\A$.

\begin{lemma}\label{lemma:impact-interpol}
  Given an ART $\Art = \tuple{N,E,\rootNode,\Lambda,R,T,\lhd}$ built
  by Algorithm \ref{alg:impact} and a node $n \in N$, for all $(n,a,m)
  \in E$, we have $\Post{\A}(\Lambda(n),a) \models \Lambda(m)$.
\end{lemma}
\proof{ We distinguish two cases. First, if $(n,a,m)$ occurs on a path
  in $\Art$ that has never been refined, then $\Lambda(m)=\top$ and
  the entailment holds trivially. Otherwise, let $(n_0,a_1,n_1),
  \ldots, (n_{k-1},a_k,n_k)$ be a path, where $n_0 = \rootNode$ and
  $(n,a,m) = (n_{i-1},a_i,n_i)$, for some $i \in [1,k]$ and, moreover,
  $a_1\ldots a_k$ was found, at some point, to be a spurious
  counterexample. Let $\tuple{\top,I_1,\ldots,I_k\equiv\bot}$ be an
  interpolant for $\mu(n_k) \equiv \Lambda(\rootNode) \wedge
  \bigwedge_{i=1}^k \Theta(n_{i-1},a,i-1) \wedge \bigwedge_{q \in R(n_k)}
  (q_k \rightarrow \bot)$. 
\qed}
...
\proof{ The first point is proved along the lines of the first point
  ...
  each moment during the execution. 

  For the second point, we prove the following invariant: each time
  Algorithm \ref{alg:impact} reaches the line \ref{ln:impact-while},
  the set of nodes $W$ in $\worklist$ contains exactly those nodes in
  the ART $\tuple{N \cup W,E,\Lambda,R,T,\lhd}$ which are
  both\begin{inparaenum}[(i)]
  \item uncovered, and such that
  \item for some $a \in \Sigma$, the edge $(n,a,s)$ is missing from $E$:
  \end{inparaenum}
  \begin{equation}\label{inv:impact}
  W = \set{n \not\in N \mid n \text{ uncovered, } \exists a \in
    \Sigma~ \forall s \in N ~.~ (n,a,s) \not\in E}
  \end{equation}
  Initially, $W = \set{\rootNode}$ and $N=E=\lhd=\emptyset$, thus the
  invariant holds trivially. Suppose that (\ref{inv:impact}) holds at
  when reaching line \ref{ln:impact-while} and some node $n$ was
  removed from $W$ and inserted into $N$. We distinguish two cases,
  either: \begin{itemize}
  \item $n$ is covered at line \ref{ln:close-worklist}, in which case
    $W$ remains unchanged, or
  %
  \item $n$ is not covered, in which case for each $a \in \Sigma$ a
    new node $s$ is inserted into $W$ (line \ref{ln:expand-end}) and
    the edge $(n,a,s)$ is inserted into $E$ (line
    \ref{ln:edge-insert}). None of these new nodes are covered and
    none of their successors inserted into $E$.
  \end{itemize}
  ...
  \ref{lemma:safety-invariant}. To this end, let $u \in \Sigma^*$ be
  an arbitrary sequence and let $v_1$ be the largest prefix of $u$
  that labels a path from $\rootNode$ to some node $n_1 \in N$. 
\qed}
