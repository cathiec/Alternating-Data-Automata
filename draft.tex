...
\proof{ The first point is proved along the lines of the first point
  ...
  W = \set{n \mid n \text{ is uncovered and } \exists a \in \Sigma~
    \forall s \in N ~.~ (n,a,s) \not\in E}
  \end{equation}
  Initially, $W = \set{\rootNode}$ and $N=E=\lhd=\emptyset$, thus the
  invariant holds trivially. Suppose that the invariant holds at some
  point, when reaching line \ref{ln:impact-while} and some node $n$
  ...
  \end{itemize}
  Then the condition (\ref{inv:impact}) holds next time the line
  \ref{ln:impact-while} is reached, thus proving the invariant.  \qed}
