...
sequence labeling the path from $\rootNode$ to $m$. A node $n \in N$
is \emph{covered by $m$} if $(n,m) \in \lhd$. We say that $n$ is just
\emph{covered} when $m$ is not important. A node $n$ is \emph{closed}
if for all nodes $m \in N$ such that $n \prec^* m$, either $\Lambda(n)
\not\models \Lambda(m)$ or $(n,m) \in \lhd$ holds. To efficiently
check that a node is closed, one may keep all the nodes from the ART
in a list, in preorder.
...
further. If $n$ has not been covered in the close phase, we check if
it is the endpoint of a counterexample candidate, which can only be
the case 
