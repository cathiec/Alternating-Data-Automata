...
words over $\vec{x}$. This definition extends the classical notion of
words from a finite alphabet with a possibly infinite alphabet
...
\section{Antichains for Emptiness}
...
antichain\footnote{Given a partial order $(D,\preceq)$ an antichain is
  a set $A \subseteq D$ such that $a \not\preceq b$ for any $a,b \in
  A$.} w.r.t. the entailment partial order. This is because, for any
$u,v \in \Sigma^*$, if $\Post{\A}(\iota,u) \models \Post{\A}(\iota,v)$
and $\Accept{\A}(uw)$ is satisfiable, then $\Accept{\A}(vw)$ is
satisfiable as well, and there is no need to store
$\Post{\A}(\iota,u)$, since the non-emptiness of $\A$ can be proved
using $v$ alone. However, in practice, the semi-algorithm using
...
interpretations, there exist infinite antichains\footnote{For
  instance, when $\Data^\I = \nat$, an infinite antichain is $q \wedge x \teq
  0, q \wedge x \teq 1, \ldots$} $\set{\Post{\A}(\phi,u) \mid u \in U}$, where
$U$ is an infinite subset of $\Sigma^*$.

A \emph{safety invariant} for $\A$ is a function $I : (Q \mapsto
\booli) \rightarrow 2^{\vec{x} \mapsto \Data^\I}$ such that, for every
boolean valuation $\beta : Q \rightarrow \booli$, the following
hold: \begin{compactitem}
\item if $\beta \models \iota$ then $\nu \in I(\beta)$ iff
  $\nu(x)=\mathbf{0}$ for all $x \in \vec{x}$,
%
\item for each $u \in \Sigma^*$, if $\beta \cup \nu \models
  \Post{\A}(\iota,u)$ then $\nu \in I(\beta)$, and
%
\item 
\end{compactitem} 
