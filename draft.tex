...
\Rightarrow \ldots \Rightarrow \phi_n(Q,\vec{x}_0 \cup \ldots
...
\item $\theta_{n+1}(Q_n) \equiv \bigwedge_{q \in Q \setminus F} (q_n
  \rightarrow \bot)$.
\end{compactitem}
The intuition is that $R_0,\ldots,R_n$ are the sets of states replaced,
$\theta_0, \ldots, \theta_n$ are the transition rules fired on the run
of $\A$ over $u$ and $\theta_{n+1}$ is the acceptance condition, which
forces the last remaining non-final states to be false.
...
are not directly suitable to producing interpolants that relate states
and data.
...
least infeasible suffix of $\lambda(n)$ is the smallest sequence $v =
...
next time $a$ is fired, it creates a node labeled $q_1$ and the next
sequence $aa$ creates a successor node $q_1$, which is covered by the
...
the root. Then the ART is rebuilt with the new predicates and
the sequence $aab$ yields an uncovered node $\top$, in Figure
...
\ref{alg:predabs} are stated by the following theorem:
...
\Lambda(m)$ for all $m \in N$ such that $m \prec^* n$.
