...
a formula $\phi$, we write $\Delta(\phi)$ and, respectively,
...
\proof{ We prove $L(\A_\cup) = L(\A_1) \cup L(\A_2)$ first, the proof
  ...
  $n>0$, we compute:
  \[\begin{array}{lcl}
  \overline{\Delta}^n(\overline{\Delta}(\iota_1,a_1\ldots a_{n-1}),a_n) & \iffeq & \text{ by induction hypothesis} \\
  \overline{\Delta}^n(\overline{\Delta(\iota_1,a_1\ldots a_{n-1})},a_n) & \iffeq & \text{ by the definition of $\overline{\phi}$} \\
  \overline{\Delta(\iota_1,a_1\ldots,a_n)}
  \end{array}\]
...
valuations (\ref{it1:inv}) and excludes the valuations satisfying the
acceptance condition (\ref{it2:inv}).  A formula $\phi(Q,\vec{x})$
...
\proof{``$\Rightarrow$'' If $\I,\beta\cup\overline\nu\cup\nu \models
  \Delta^n(\phi,a)$, for some valuations $\beta : Q \rightarrow
  \booli$ and $\overline\nu : \overline{\vec{x}} \rightarrow
  \Data^\I$, $\nu : \vec{x} \rightarrow \Data^\I$, then we build a
  valuation $\beta' : Q' \rightarrow \booli$ such that
...
\proof{ Let $\pi(u) \equiv \theta_0(Q_0) \wedge \theta_1(Q_0 \cup
  ...
  $\pi(u) \equiv \bigwedge_{i=0}^{n+1} \theta_i$ is satisfiable, we
  obtain a model for $\Accept{\A}(u) \equiv \Post{\A}(\iota,u) \wedge
  \theta_{n+1}[Q/Q_n]$.
  ...
  \end{compactitem}
  We prove that $\AbsPost{\A}(\iota,a_1\ldots a_n) \models I_{n+1}$ by
  ...
  \models I_1$. For the induction step $n>0$, we compute:
  \[\begin{array}{l}
...
\emph{abstract reachability tree} (ART) whose edges are labeled with
input events and each node $n$ is labeled with the formula
...
Formally, an ART $\Art = \tuple{N,E,\rootNode,\Lambda,\lhd}$ is a
tree, where $N$ is a set of nodes, $E \subseteq N \times \Sigma \times
N$ is a set of edges, $\rootNode \in N$ is the root of the directed
tree $(N,E)$, $\Lambda : N \rightarrow \Form^+(Q,\vec{x})$ is a
labeling of the nodes with formulae, such that $\Lambda(\rootNode) =
\iota$ and $\lhd \subseteq N \times N$ is a set of covering
edges. Then each node $n \in N$ corresponds to a unique path from the
root to $n$, labeled by a sequence $u$ of input events. The least
unfeasible suffix of $u$ is the smallest sequence $v = a_1 \ldots
a_k$, such that $u = wv$, for some $w \in \Sigma^*$ and the formula
(\ref{eq:pivot}) below is unsatisfiable. The \emph{pivot} of $u$ is
the node $p$ corresponding to the start of $v$ in $u$.
...
\proof{To prove the first point, assume w.l.o.g. that $\Sigma =
  ...
  of nodes in $\worklist$ contains all the frontier nodes in the ART
  $\tuple{N\cup W,E,\rootNode,\Lambda,\lhd}$ which are not covered by some node
  in $N$, namely that:
...
$\rootNode$ to $n$. In addition, we require that an interpolant
$\tuple{\top,I_0,\ldots,I_k,\bot}$ for the insatisfiable interpolation
problem $\mu(n)$ does not have negative occurrences of states,
i.e.\ $I_i \in \Form^+(Q,\vec{x})$, for all $i \in [0,k]$. Such an
interpolant can always be built, as showed below:
...
\begin{proposition}\label{prop:positive-interpol}
  ...
  $\tuple{\top,I_0,\ldots,I_n,\bot}$ for $\pi(u)$, such that $I_i
  \in \Form^+(Q,\vec{x})$, for all $i \in [0,n]$.
\end{proposition}
...
{\scriptsize\begin{algorithmic}[1] 
  ...
  \State let $\tuple{\top,I_0,\ldots,I_k,\bot}$ be an
  interpolant for $\mu(n)$ with $I_0,\ldots,I_k \in
  \Form^+(Q,\vec{x})$
...
%
Let $u$ be the label of the path from the root node $\rootNode$ to
$n$. If $\mu(n)$ is satisfiable, then $u$ is a feasible
counterexample, in which case a model of $\mu(n)$ is obtained and a
word $w \in L(\A)$ returned. Otherwise, $u$ is a spurious
...
$\mu(n)$ is used to strenghten the labels of all the ancestors of $n$,
by conjoining the formulae of the interpolant to the existing
labels. For technical reasons, related to proving the correctness of
Algorithm \ref{alg:impact}, we assume that all formulae from the
interpolant for $\mu(n)$ have only positive occurrences of the
states\footnote{According to Proposition \ref{prop:positive-interpol},
  building such an interpolant is always possible} in $Q$.
...
Finally, if $n$ is still uncovered (i.e.\ it has not been previously
covered during the refinement phase) we expand $n$ (lines
...
the worklist. The label of the new node is $\top$, its replacement set
$R(s)$ is computed based on $R(n)$ and the label of the edge $(n,a,s)$
is $\Theta(n,a,k)$, where $k$ is the depth of $n$ in $\Art$.
...
\proof{ We distinguish two cases. First, if $(n,a,m)$ occurs on a path
  ...
  $\langle\top,I^\omega_1,\ldots,I^\omega_k\equiv\bot\rangle$ be an
  interpolant for $\mu(n_k) \equiv \Lambda(\rootNode) \wedge
  \bigwedge_{i=1}^k \Theta(n_{i-1},a,i-1) \wedge \bigwedge_{q \in
    R(n_k)} (q_k \rightarrow \bot)$, such that $I^\omega_i \in
  \Form^+(Q,\vec{x})$, for all $i \in [1,k]$. According to Proposition
  \ref{prop:positive-interpol}, it is always possible to build such an
  interpolant, when $\mu(n_k)$ is unsatisfiable. By Proposition
...
\begin{theorem}\label{thm:impact}
  ...
  $L(\A) = \emptyset$.
\end{theorem}
\proof{ The first point is proved along the lines of the first point
  of Theorem \ref{thm:predabs}. In particular, it is easy to see that
  each call to $\Call{Close}{}$ terminates, because $N$ is finite at
  each moment during the execution. For the second point, we prove
  that each time Algorithm \ref{alg:impact} reaches the line
  \ref{ln:impact-while}, we have:
  ...
  \ref{ln:impact-while} is reached, thus it is invariant. If Algorithm
  \ref{alg:impact} returns $\true$ then $W = \emptyset$ and, by
  (\ref{inv:impact}), for each node in $n \in N$ one of the following
  hold: \begin{compactitem}
  ...
  \end{compactitem}
  Moreover, we have $u = v'_1 \ldots v'_\ell$, thus $\Post{\A}(\iota,u)
  \models \Lambda(m_\ell)$ and we are done showing that $\bigvee_{n\in
    N} \Lambda(n)$ is an invariant. To prove that it is, moreover, a
  safety invariant, 
\qed}
