...
\cite{mcmillan06}. These algorithms are proven to terminate and return
a word from the language, if the language is not empty, but
termination is not guaranteed when the language is empty. Finally, we
prove termination in the empty case, when the transition
...
\begin{figure}[htb]
\begin{tabular}{cc}
\includegraphics[scale=0.5]{figures/PA1.pdf} & \includegraphics[scale=0.5]{figures/PA2.pdf} \\
\footnotesize{(a)} & \footnotesize{(b)} \\
\\
\includegraphics[scale=0.5]{figures/PA3.pdf} & \includegraphics[scale=0.5]{figures/PA4.pdf} \\
...
is $\top$ (a). The pivot of $a$ is the root of the ART and the
spuriousness check results in the predicates $\set{q_0,q_1}$, which
are added to $\preds$ and the $\top$ node is deleted. The next time
$a$ is fired, it creates a node labeled $q_1$ and the next sequence
$aa$ creates another node $q_1$ which is covered by the first,
depicted with a dashed arrow (b). The third sequence is $ab$, which
results in a new node $\top$ and triggers a spuriousness check. The
new predicate is $x\leq0\wedge q_2 \wedge y\geq0$ and the pivot is
again the root. Then the entire ART is rebuild with the new predicates
and the sequence $aab$ results in an uncovered node $\top$ (c). The
new pivot is the endpoint of $a$ and the new predicates are $q_1\wedge
q_2$ and $y>x-1 \wedge q_2$. Finally, the ART is rebuilt from the
pivot node and the result is a tree in which all nodes are covered,
thus proving the emptiness of the automaton. \qed
