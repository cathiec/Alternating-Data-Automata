...
The solution we adopt is the use of abstraction to ensure termination
of invariant computations. However, it is worth pointing out from the
...
\[\begin{array}{llcl}
\AbsPost{\A}(\phi,\varepsilon) \equiv \phi & 
...
\proof{ Let $u = a_1\ldots a_n$ and  $\pi(u) \equiv \theta_0(Q_0) \wedge
  ...
  (\ref{it2:progress}) If $\tuple{\top,I_1,\dots,I_{n+1},\bot}$ is an
    ...
    compute:
    \[\begin{array}{lcl}
    \AbsPost{\A}(\iota,a_1\ldots a_n)[Q_n/Q] & \equiv & \text{ def. of $\AbsPost{\A}$} \\
    \exists \vec{x}_{n-1} ~.~ \abs{\Delta^n(\AbsPost{\A}(\iota,a_1\ldots a_{n-1}),a_n)}[Q_n/Q] & \models \\
    \exists Q_{n-1}  \exists \vec{x}_{n-1} ~.~ \AbsPost{\A}(\iota,a_1\ldots a_{n-1})[Q_{n-1}/Q] \wedge \theta_n & \models & \text{ ($\Asterisk$)} \\
    \exists Q_{n-1}  \exists \vec{x}_{n-1} ~.~ I_{n-1}[Q_{n-1}/Q] \wedge \theta_n & \models & \text{ ind. hyp.} \\ 
    I_n[Q_n/Q] & \hfill\text{\qed}
    \end{array}\]
...
{\scriptsize\begin{algorithmic}[1]
  ...
  \State let $s$ be a fresh node and $\Lambda \leftarrow \Lambda \cup
  \set{(s,\phi)}$
