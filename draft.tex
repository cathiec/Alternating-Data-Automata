...
\rightarrow \bot)$ is satisfiable ?  Observe that, since we ask a
satisfiability query, the final states of $\A$ need not be
constrained\footnote{ Since each state occurs positively in
  $\Accept{\A}(u)$, this formula has a model iff it has a model with
  every $q \in F$ set to true.}. A na\"ive semi-algorithm enumerates
all finite sequences and checks the satisfiability of $\Accept{\A}(u)$
for each $u \in \Sigma^*$, using a decision procedure for the theory
$\theory(\sig,\I)$.
...
there exist infinite antichains, e.g.\ $q \wedge x \teq 0, q \wedge x
\teq 1, \ldots$ for $\Data^\I = \nat$.
...
$\beta\cup\nu \models \phi \iff \nu \in \Inv(\beta)$. If $\Inv$
satisfies only the first point above, we call it simply an
...
configuration $\iota = \Post{\A}(\iota,\varepsilon)$, in which the
data variables are unconstrained, and over-approximates the set of
reachable valuations (\ref{it1:inv}), while excluding those valuations
which satisfy the acceptance condition (\ref{it2:inv}).
...
which are not necessarily safety invariants. To meet the latter
condition, we resort to \emph{counterexample guided abstraction
  refinement} (CEGAR).
...
{\footnotesize\[ \AbsPost{\A}(\phi,\varepsilon) \equiv \phi
\hspace*{2mm}
...
\begin{equation}\label{eq:interpolation-problem}
\pi(u) \equiv \theta_0(Q_0) \wedge \theta_1(Q_0 \cup Q_1,\vec{x}_0
\cup \vec{x}_1) \wedge \ldots \wedge \theta_n(Q_{n-1} \cup
Q_n,\vec{x}_{n-1} \cup \vec{x}_n) \wedge \theta_{n+1}(Q_n)
\end{equation} 
...
transition function of $\A$. The main idea behind the above definition
of the interpolation problem is that we would like to obtain an
interpolant $\tuple{\top,I_1(Q), I_2(Q,\vec{x}), \ldots,
  I_{n}(Q,\vec{x}),\bot}$ whose formulae \emph{combine states with the
  data constraints that must hold locally}, whenever the control
...
interpolants that can be applied locally. Furthermore, the abstraction
defined by the interpolants generated in this way can also
...
\begin{lemma}\label{lemma:progress}
  ...
  \end{compactenum}
\end{lemma}
\proof{ Let $u = a_1\ldots a_n$ and  $\pi(u) \equiv \theta_0(Q_0) \wedge
  \theta_1(Q_0 \cup Q_1,\vec{x}_0 \cup \vec{x}_1) \wedge \ldots \wedge
  \theta_n(Q_{n-1} \cup Q_n,\vec{x}_{n-1} \cup \vec{x}_n) \wedge
  \theta_{n+1}(Q_n)$ in the following. 
  ...
  $\bigcup_{i=0}^n \beta_i \cup \bigcup_{i=0}^n \nu_i \models
  ...
  under valuation $\beta\cup\nu_{n-1}\cup\nu_n$ is not important for
  $q \in Q$ such that $\beta(q) = \false$. We have thus obtained a
  ...
  (\ref{it2:progress}) If $\tuple{\top,I_1,\dots,I_{n+1},\bot}$ is an
    ...
    \item $\theta_0 \models I_1[Q_0/Q]$, 
      %
    \item $I_k[Q_{k-1}/Q,\vec{x}_{k-1}/\vec{x}] \wedge \theta_k \models I_{k+1}[Q_k/Q,\vec{x}_k/\vec{x}]$, for all $k\in[1,n]$. 
      %
    ...
    compute:
    \[\begin{array}{rcll}
    \AbsPost{\A}(\iota,a_1\ldots a_n)[Q_n/Q] & \equiv & \exists \vec{x}_{n-1} ~.~ \abs{\Delta^n(\AbsPost{\A}(\iota,a_1\ldots a_{n-1}),a_n)}[Q_n/Q] & \text{ def. of $\AbsPost{\A}$} \\
    & \models & \exists Q_{n-1}  \exists \vec{x}_{n-1} ~.~ \AbsPost{\A}(\iota,a_1\ldots a_{n-1})[Q_{n-1}/Q] \wedge \theta_n & \text{ ($\Asterisk$)} \\
    & \models & \exists Q_{n-1}  \exists \vec{x}_{n-1} ~.~ I_{n-1}[Q_{n-1}/Q] \wedge \theta_n & \text{ by ind. hyp.} \\ 
    & \models & I_n[Q_n/Q] & \hfill\text{\qed}
    \end{array}\]
    We prove the final point ($\Asterisk$). If
    $\beta_n\cup\nu_{n-1}\cup\nu_n \models
    \abs{\Delta^n(\AbsPost{\A}(\iota,a_1\ldots a_{n-1}),a_n)}[Q_n/Q]$,
    for some valuations $\beta_{n-1} : Q_{n-1} \rightarrow \booli$ and
    $\nu_i : \vec{x}_i \rightarrow \Data^\I$, for $i=n-1,n$, then we
    build a valuation $\beta_{n-1}$ such that $\beta_{n-1}\cup\beta_n
    \cup \nu_{n-1}\cup\nu_n \models \AbsPost{\A}(\iota,a_1\ldots
    a_{n-1})[Q_{n-1}/Q] \wedge \theta_n$, as follows. For each
    occurrence of a subformula $\Delta(q,a_n)$ in
    $\AbsPost{\A}(\iota,a_1\ldots a_{n-1})$ we set
    $\beta_{n-1}(q_{n-1}) = \true$ if $\beta_n\cup\nu_{n-1}\cup\nu_n
    \models \Delta(q,a_n)[Q_n/Q]$ and $\beta_{n-1}(q_{n-1}) = \false$,
    otherwise. Since there are no negated occurrences of such
    subformulae, the definition of $\beta_{n-1}$ is consistent, and
    the check $\beta_{n-1}\cup\beta_n \cup \nu_{n-1}\cup\nu_n \models
...
automata. Algorithm\footnote{Although termination is not guaranteed,
  we call it an algorithm for conciseness.}  \ref{alg:predabs} builds
...
\begin{algorithmic}[1]
  ...
  \tuple{\rootNode}$, $\Art = \tuple{N,E,\Lambda,\lhd}$, where $N \leftarrow
  \emptyset$, $E \leftarrow \emptyset$, $\Lambda \leftarrow
  \set{(\rootNode,\iota)}$ and $\lhd \leftarrow \emptyset$
  ...
  \If{$\Lambda(n) \wedge \bigwedge_{q \in Q \setminus F} (q
    \rightarrow \bot)$ is satisfiable} \Comment{counterexample found}
  ...
  \State let $\beta \cup \nu_1 \cup \ldots \cup \nu_k \models
  \Accept{\A}(u)$, where $\beta : Q \rightarrow \booli$,
  $\nu_1,\ldots,\nu_k : \vec{x} \rightarrow \Data^\I$
  ...
  \State let $p \leftarrow \Call{FindPivot}{u,\Art}$
  ...
  \State let $\phi \leftarrow \AbsPost{\A}(\Lambda(n),a)$

  \If{exist $m \in N$ such that $\phi \models \Lambda(m)$} 

  \State $\lhd \leftarrow \lhd \cup \set{(n,m)}$ \Comment{$n$ covers $m$}

  \Else

  \State let $s \leftarrow \Call{NewNode}{}$ and $\Lambda \leftarrow
  \Lambda \cup \set{(s,\phi)}$
  ...
  \For{$m \in N$ such that $(m,r) \in E$}

  \State let $\lhd \leftarrow \lhd \cup \set{(m,s)}$ \Comment{redirect covered children from $R$ into $s$}

  \EndFor

  \For{$(m,r) \in \lhd$}

  \State let $\lhd \leftarrow \lhd \cup \set{(m,s)}$ \Comment{redirect covered nodes from $R$ into $s$}
  ...
  \State enqueue $s$ into $\worklist$ \Comment{recompute the subtree rooted at $s$}
  ...
  \State {\bf return} $\true$
\end{algorithmic}
\caption{Lazy Predicate Abstraction for Checking Emptiness of ADA}
\label{alg:predabs}
\end{algorithm}

Formally, let $\Art = \tuple{N,E,\Lambda,\lhd}$ be a tree, where $N$
is a set of nodes, $E \subseteq N \times \Sigma \times N$ is a set of
edges, $\Lambda : N \rightarrow \Form^+(Q,\vec{x})$ is a labeling of
the nodes with formulae, and $\lhd \subseteq N \times N$ is a set of
covering edges. The root of the tree is labeled with the initial
...
\begin{equation}\label{eq:pivot}
  \overline{\pi}(a_1\ldots a_k) \equiv \Lambda(p)[Q_0/Q] \wedge
  \theta_1(Q_0 \cup Q_1,\vec{x}_0 \cup \vec{x}_1) \wedge \ldots \wedge
  \theta_{k+1}(Q_k)
\end{equation}
...
iteration to build a ART. We keep newly expanded nodes of $\Art$ in a
...
generated node is covered by a node in $N$, it is not added to the
worklist, thus cutting the current branch of the search. Formally, for
any two nodes $n,m \in N$, we have $n \lhd m$ iff
