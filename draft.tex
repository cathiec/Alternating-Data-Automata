...
\proof{To prove the first point, assume w.l.o.g. that $\Sigma =
  ...
  $\tuple{N\cup W,E,\Lambda,\lhd}$ which are not covered by some node
  in $N$, namely that:
...
\proof{ The first point is proved along the lines of the first point
  ...
  \ref{lemma:safety-invariant}. To show that $\bigvee_{n \in N}
  \Lambda(n)$ defines an invariant, let $u \in \Sigma^*$ be an
  arbitrary sequence. \qed}
